...
function Navbar() {
    ...
    const {search,menu,categoriesModal,brandsModal,imageSearch} = useSelector(state=>state.modals)
    ...
    return (
        <>
            ...
            <motion.nav initial={{ y: -250 }} animate={{ y: 0 }} transition={{ type: "spring" }} className="shadow-sm px-6 py-4 md:px-8  flex items-center justify-between font-sans relative">
                ...
                <motion.div className={`pt-7 md:pt-0 pb-7 absolute z-50 bg-white shadow ${(menu && !isCartOpen) ? "" : "translate-y-[-200%]"} min-h-[40%] right-0 top-20 w-full md:translate-y-0 md:shadow-none md:z-0 md:flex md:bg-transparent md:pb-0 md:static md:min-h-fit md:w-auto transition-all duration-300 ease-in`}>
                    <ul className="flex flex-col gap-12 md:flex-row items-center md:flex md:gap-6 text-lg font-medium">
                        ...
                        <motion.li ref={brandRef} id="brands"    onClick={() => { dispatch(setBrandsModal(!brandsModal)); dispatch(setCategoriesModal(false))}} whileHover={{ scale: 1.1 }} className="hover:text-sky-600 cursor-pointer">
                            Brands
                            {isMobile && brandsModal && <motion.div initial={{ y: -20, scaleX: 0.4, opacity: 0.7 }} animate={{ scaleX: 1, y: 0, opacity: 1 }} transition={{ type: "spring" }} id="brandsHover" style={{ left: brandsPostion }} className={`z-[9999] absolute bg-white divide-y divide-gray-100 rounded-lg shadow w-44 w-min-44`}>
                        ...
                        <motion.li ref={categoryRef} id="category" onClick={() => { dispatch(setCategoriesModal(!categoriesModal)); dispatch(setBrandsModal(false)) }} whileHover={{ scale: 1.1 }} className="hover:text-sky-600 cursor-pointer">
                            Category
                            {categoriesModal && isMobile && <motion.div initial={{ y: -20, scaleX: 0.4, opacity: 0.7 }} animate={{ scaleX: 1, y: 0, opacity: 1 }} transition={{ type: "spring" }} id="categoryHover" style={{ left: categoryPostion }} className={`z-[9999] absolute bg-white divide-y divide-gray-100 rounded-lg shadow w-min-44`}>
            ...
            {brandsModal && !isMobile && <motion.div initial={{ y: -20, scaleX: 0.4, opacity: 0.7 }} animate={{ scaleX: 1, y: 0, opacity: 1 }} transition={{ type: "spring" }} id="brandsHover" style={{ left: brandsPostion }} className={`z-[9999] absolute bg-white divide-y divide-gray-100 rounded-lg shadow w-44 w-min-44`}>
                <ul className="py-2 text-sm text-gray-700 grid-rows-5 grid-flow-col" aria-labelledby="dropdownHoverButton">
                    {brands.map((brand, i) => (
                        <li key={i}>
                            <Link to={`brands/${brand}`} className="block px-4 py-2 hover:bg-gray-100 hover:rounded-lg hover:text-sky-600">{brand}</Link>
